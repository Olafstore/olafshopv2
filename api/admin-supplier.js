...
export function createReadHandler({env=process.env,fetcher=fetch}={}) {
  return async function handler(req,res) {
    ...
    if(req.query?.action && !['products','test'].includes(req.query.action))return res.status(404).json({success:false,error:{code:'ACTION_NOT_FOUND',message:'Read-only endpoint'}});
    try {
