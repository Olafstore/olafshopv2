...
const PUBLIC_PRODUCT_FIELDS='id,name,description,image,platform,region,stock,selling_price,full_price,denuvo,steam';
...
export function publicSupplierProduct(row){
  return {id:row.id,name:cleanText(row.name),description:cleanText(row.steam?.description_th || row.description || row.steam?.description),
    image:imageURL(row.image),platform:'steam',region:cleanText(row.region),available:row.stock>0,
    ...
    requirements:cleanText(row.steam?.pc_requirements?.minimum)};
}
...
export function createSupplierShop({env=process.env,db=createShopDb({env}),client=create499kFulfillmentClient({env})}={}){
  function ready(){
  ...
  return {
    ...
    async catalog(){
      ...
      return {checkoutEnabled,products:rows.map(publicSupplierProduct)};
    },
    async checkout(userId,{productId,requestId,paymentMethod}){
      ready();
      ...
      const result=await db.rpc('server_create_supplier_checkout',{p_user_id:userId,p_product_id:productId,p_request_id:requestId,
        p_quote_cost:quote.price,p_quote_stock:quote.stock,p_payment_method:paymentMethod});
