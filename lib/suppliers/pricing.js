...
// Requested policy: profit is 50% of cost (33.33% gross margin), rounded up.
export function priceAt50PercentMarkup(cost) {
  if(!/^\d{1,10}(\.\d{1,2})?$/.test(String(cost)))throw new Error('INVALID_SUPPLIER_COST');
  const [whole,fraction='']=String(cost).split('.');
  const cents=BigInt(whole)*100n+BigInt(fraction.padEnd(2,'0'));
  if(cents<=0n)throw new Error('INVALID_SUPPLIER_COST');
  const selling=(cents*3n+1n)/2n;
  const money=n=>`${n/100n}.${String(n%100n).padStart(2,'0')}`;
  return {selling_price:money(selling),profit_amount:money(selling-cents),profit_percent:Number((selling-cents)*100000000n/cents)/1000000};
}
