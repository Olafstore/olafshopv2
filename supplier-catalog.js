...
(() => {
  ...
  let verifiedMedia={},mediaPending;
  const loadMedia=()=>mediaPending||(mediaPending=fetch('assets/supplier-steam-media-v18.json',{signal:globalThis.AbortSignal?.timeout?.(6000)}).then(r=>r.ok?r.json():null).then(j=>{verifiedMedia=j?.apps||{};}).catch(()=>{}));
  function steamFields(s){return s?{image:s.headerImage,heroImage:s.screenshots?.[0]||s.headerImage,gallery:s.screenshots||[],description:s.description,shortDescription:s.shortDescription,systemRequirements:{minimum:s.requirements?.minimum||[],recommended:s.requirements?.recommended||[]},ageStatus:s.ageStatus,ageRestricted:s.ageRestricted,requiredAge:s.requiredAge}:{};}
  const legacyRows=new Map(),legacyAges=new Map();
  const protect=p=>window.OlafAgeGate?.protect(p)||p;
  const mapLegacy=p=>{if(!p)return p;legacyRows.set(p.id,p);return protect({...p,ageLocked:false,...steamFields(verifiedMedia[window.OlafAgeGate?.steamId(p)]),...legacyAges.get(p.id)});};
  async function legacyDetail(id,cached=false){
    ...
    if(verifiedMedia[appId])return mapLegacy(p);
  ...
  const mapProduct=(p,checkoutEnabled=true)=>{const s=verifiedMedia[p.steamAppId];if(s&&p.steamMetadataStatus!=='ready')p={...p,image:s.headerImage,screenshots:s.screenshots,description:s.description,shortDescription:s.shortDescription,requirements:s.requirements.minimum.join('\n'),recommendedRequirements:s.requirements.recommended.join('\n'),ageStatus:s.ageStatus,ageRestricted:s.ageRestricted,steamMetadataStatus:'verified-snapshot'};return protect({
  ...
  if(document.addEventListener&&typeof IntersectionObserver!=='undefined')document.addEventListener('DOMContentLoaded',()=>{
    const observer=new IntersectionObserver(entries=>{for(const entry of entries){if(!entry.isIntersecting)continue;observer.unobserve(entry.target);
      ...
      if(!id||queued.has(id))continue;queued.add(id);
      const known=snapshot.find(p=>p.id===id)||legacyRows.get(id);
      if(known&&verifiedMedia[window.OlafAgeGate?.steamId(known)||known.steamAppId])continue;
      queue.push(id);drain();
    }},{rootMargin:'150px'});
