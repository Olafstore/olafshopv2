...
const safeOrder=(s,o)=>({id:s.order_id,name:s.product_name,image:imageURL(s.product_image),price:Number(s.selling_price),
  ...
  total:Number(o.total),pointsRedeemedAmount:Number(o.points_redeemed_amount||0),expiresAt:o.expires_at,createdAt:s.created_at,hasSlip:Boolean(o.payment_slip_path),
...
export function createSupplierShop({env=process.env,db=createShopDb({env}),client=create499kFulfillmentClient({env}),sync=()=>syncSupplierCatalog({env,db}),steam=getSteamMetadata}={}){
  ...
  async function owned(orderId,userId){
    ...
    const [o]=await db.select('orders',{select:'id,user_id,order_number,status,payment_status,total,points_redeemed_amount,expires_at,payment_method,payment_slip_path',id:`eq.${orderId}`,user_id:`eq.${userId}`,limit:'1'});
  ...
  return {
    ...
    async checkout(userId,{productId,requestId,paymentMethod,expectedPrice,pointsToUse=0}){
      ready();
      if(typeof pointsToUse!=='number'||!Number.isFinite(pointsToUse)||pointsToUse<0||Math.abs(pointsToUse*100-Math.round(pointsToUse*100))>0.000001)throw new ShopError('INVALID_CHECKOUT',400);
      const complete=async(s,o)=>{
        if(Number(o.total)===0&&o.payment_status==='verified'&&s.state!=='delivered'){
          try{await this.fulfill(userId,s.order_id);const latest=await owned(s.order_id,userId);return safeOrder(latest.s,latest.o);}
          catch{return {...safeOrder(s,o),needsSupport:true};}
        }
        return safeOrder(s,o);
      };
      ...
      if(prior){const {s,o}=await owned(prior.order_id,userId);if(s.product_id!==productId||o.payment_method!==paymentMethod||Number(o.points_redeemed_amount||0)!==pointsToUse)throw new ShopError('IDEMPOTENCY_CONFLICT',409);return complete(s,o);}
      ...
      const result=await db.rpc(pointsToUse>0?'server_create_supplier_checkout_v3':'server_create_supplier_checkout_v2',{p_user_id:userId,p_product_id:productId,p_request_id:requestId,
        p_quote_cost:quote.price,p_quote_stock:quote.stock,p_payment_method:paymentMethod,p_expected_price:expectedPrice,...(pointsToUse>0?{p_points_to_use:pointsToUse}:{})});
      const {s,o}=await owned(result.order.id,userId);return complete(s,o);
    },
    async orders(userId){
      const rows=await db.select('supplier_checkouts',{select:'*',user_id:`eq.${userId}`,order:'created_at.desc',limit:'100'});
      if(!rows.length)return [];
      const orders=await db.select('orders',{select:'id,user_id,order_number,status,payment_status,total,points_redeemed_amount,expires_at,payment_method,payment_slip_path',id:`in.(${rows.map(r=>r.order_id).join(',')})`,user_id:`eq.${userId}`,limit:'100'});
