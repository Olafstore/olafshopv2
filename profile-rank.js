(() => {
  ...
  function render(state) {
    ...
    root.innerHTML = `<section class="member-ranks" aria-label="แรงค์ประจำเดือน">
      ...
      <label class="rank-display-toggle"><input type="checkbox" data-rank-display ${displayState?.show?'checked':''} ${displayState===null?'disabled':''}>แสดงแรงค์ข้างชื่อในโปรไฟล์และเมนูผู้ใช้</label><span class="rank-display-status" role="status">${displayState===null?'โหลดตัวเลือกไม่สำเร็จ กรุณาตรวจ SQL การแสดงแรงค์':''}</span>
