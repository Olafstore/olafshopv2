...
export async function getSteamMetadata(appId,{fetcher=fetch,language='thai',now=Date.now}={}){
  // Steam's SHOGUN 2 store alias 201270 resolves to canonical app 34330.
  if(String(appId)==='201270')appId='34330';
  ...
  const promise=(async()=>{
    ...
    // Some responses use a different envelope key; only accept the exact ID
    // inside the record, never an unrelated first result or a name match.
    const item=Object.values(payload||{}).find(entry=>entry?.success&&String(entry.data?.steam_appid)===String(appId));
    const d=item?.data;if(!item||!d)throw new Error('STEAM_APP_NOT_FOUND');
