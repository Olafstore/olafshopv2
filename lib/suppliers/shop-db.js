...
const dbCodes=['ORDER_NOT_FOUND','IDEMPOTENCY_CONFLICT','SUPPLIER_PRICE_CHANGED','SUPPLIER_OUT_OF_STOCK',
  ...
  'SUPPLIER_FULFILLMENT_BUSY','SUPPLIER_LEASE_MISMATCH','GUARD_BUSY','GUARD_WINDOW_CLOSED','SUPPLIER_RATE_LIMITED','POINT_BALANCE_INSUFFICIENT','INVALID_CHECKOUT'];
  ...
  async function request(path,{method='GET',body,token}={}) {
    ...
    try {
      ...
      if(response.status===429){
        const error=new ShopError('SUPPLIER_DATABASE_RATE_LIMITED',429);
        error.retryAfter=Math.min(3600,Math.max(1,Number(response.headers.get('retry-after'))||60));throw error;
      }
  ...
  return {
    ...
    rpc:(name,args)=>{
      if(!['server_create_supplier_checkout','server_claim_supplier_checkout','server_finish_supplier_checkout',
        ...
        'server_supplier_sync_claim','server_supplier_sync_finish','server_supplier_quote','server_create_supplier_checkout_v2','server_create_supplier_checkout_v3'].includes(name))throw new ShopError('INVALID_DB_TARGET');
