(() => {
  ...
  let root, pending=false, refreshTimer;
  const visible = () => root && !document.hidden && document.getElementById('panel-overview')?.classList.contains('is-active');
  function render(state) {
    ...
    root.innerHTML = `<section class="member-ranks" aria-label="แรงค์ประจำเดือน">
      ...
      <div class="rank-track" role="list">${ranks.map(([id,name,threshold,color],index)=>`<div role="listitem" class="rank-emblem ${state.rank===index+1?'is-current':''}" style="--rank-color:${color}" ${state.rank===index+1?'aria-current="true"':''}>
