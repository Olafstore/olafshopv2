...
const actions={catalog:'GET',product:'GET',diagnose:'GET',quote:'POST',checkout:'POST',orders:'GET',order:'GET',delivery:'POST',guard:'POST',fulfill:'POST',publish:'POST',readiness:'GET'};
...
export function createShopHandler({env=process.env,fetcher=fetch,db=createShopDb({env,fetcher}),shop=createSupplierShop({env,db}),admin=requireSupplierAdmin}={}){
  return async(req,res)=>{
    ...
    res.setHeader('X-OLAF-Supplier-Revision','supplier-media-v4');
