...
export function createShopDb({env=process.env,fetcher=fetch}={}) {
  async function request(path,{method='GET',body,token}={}) {
    ...
    try {
      ...
      if(!response.ok){const code=dbCodes.find(c=>data?.message===c);throw new ShopError(code || 'SUPPLIER_DATABASE_UNAVAILABLE',code==='ORDER_NOT_FOUND'?404:code?409:503);}
      return data;
  ...
  return {
    ...
    rpc:(name,args)=>{
      if(!['server_create_supplier_checkout','server_claim_supplier_checkout','server_finish_supplier_checkout',
        ...
        'server_supplier_guard_finish','server_publish_499k_storefront','server_supplier_runtime_status'].includes(name))throw new ShopError('INVALID_DB_TARGET');
