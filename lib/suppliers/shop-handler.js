...
const publicCodes=new Set(['AUTH_REQUIRED','ADMIN_REQUIRED','ADMIN_CHECK_UNAVAILABLE','ACTIVE_ACCOUNT_REQUIRED','ORDER_NOT_FOUND','INVALID_CHECKOUT','IDEMPOTENCY_CONFLICT',
  ...
  'DELIVERY_NOT_READY','INVALID_GUARD_REASON','GUARD_BUSY','GUARD_WINDOW_CLOSED','SUPPLIER_RATE_LIMITED','POINT_BALANCE_INSUFFICIENT',
  'RATE_LIMITED','SUPPLIER_DATABASE_RATE_LIMITED','CODE_LIMIT_REACHED','CODE_UNAVAILABLE','ORDER_REFUNDED',
  ...
  return async(req,res)=>{
    ...
    try{
      ...
      if(action==='catalog')data=await shop.catalog();
        ...
        if(action==='checkout')data=await shop.checkout(userId,{productId:body.productId,requestId:body.requestId,paymentMethod:body.paymentMethod,expectedPrice:body.expectedPrice,pointsToUse:body.pointsToUse});
