...
export function publicSupplierProduct(row){
  return {id:row.id,name:cleanText(row.name),description:cleanText(row.steam?.description_th || row.description || row.steam?.description),
    image:imageURL(row.image),platform:'steam',region:cleanText(row.region),available:row.stock>0,stock:Math.max(0,Number(row.stock)||0),
...
export function createSupplierShop({env=process.env,db=createShopDb({env}),client=create499kFulfillmentClient({env}),sync=()=>syncSupplierCatalog({env,db})}={}){
  function ready(){
  ...
  return {
    ...
    async product(productId){
      ...
      return publicSupplierProduct(row);
    },
    async readiness(){
    ...
    async catalog(){
      ...
      return {checkoutEnabled:checkoutEnabled&&fresh,pricesFresh:fresh,products:rows.map(publicSupplierProduct)};
    },
