// Live access is read-only. The only POST is a fixed, test-key-only sandbox order.
...
export function supplierKey(env=process.env) {
  if(typeof window!=='undefined')throw new SupplierConnectionError('SERVER_ONLY',503);
  return typeof env.SUPPLIER_499K_API_KEY==='string'?env.SUPPLIER_499K_API_KEY.trim():'';
}

// Both modes are documented by 499K. Select exactly one on the server; never
// retry a purchase using another header after an ambiguous response.
export function supplierHeaders(env=process.env) {
  const key=supplierKey(env);
  const mode=env.SUPPLIER_499K_AUTH_MODE || 'x-api-key';
  if(!/^499k_(test|live)_[A-Za-z0-9_-]+$/.test(key) || !['x-api-key','bearer'].includes(mode))
    throw new SupplierConnectionError('SUPPLIER_CONFIG_REQUIRED',503);
  return {...(mode==='bearer'?{Authorization:`Bearer ${key}`}:{'x-api-key':key}),Accept:'application/json'};
}

// Safe metadata only, for an authenticated admin; never return the key or its fragments.
export function supplierConfigStatus(env=process.env) {
  const key=supplierKey(env);
  const match=/^499k_(test|live)_[A-Za-z0-9_-]+$/.exec(key);
  return {
    revision:'supplier-key-check-v2',
    keyPresent:Boolean(key),
    keyType:match?(match[1]==='test'?'sandbox':'live'):(key?'invalid':'missing'),
    outerWhitespaceRemoved:Boolean(key && key!==env.SUPPLIER_499K_API_KEY),
    importEnabled:env.SUPPLIER_499K_IMPORT_ENABLED==='true',
    baseUrlValid:env.SUPPLIER_499K_BASE_URL==='https://store.499k-network.com',
    databaseConfigured:Boolean(env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY),
    deploymentEnvironment:['production','preview','development'].includes(env.VERCEL_ENV)?env.VERCEL_ENV:'unknown'
  };
}
...
async function readEnvelope(response, maxBytes = 65536) {
  ...
  if (!reader&&!iterator) throw new SupplierConnectionError('SUPPLIER_INVALID_RESPONSE');
  let size = 0;
  const chunks = [];
  try {
    while (true) {
    ...
    const data = JSON.parse(Buffer.concat(chunks).toString('utf8').replace(/^\uFEFF/,''));
    ...
    if(reader)await reader.cancel().catch(() => {});else await iterator.return?.();
  }
}

const providerErrors = new Set(['UNAUTHORIZED','KEY_REVOKED','CLIENT_NOT_APPROVED','CLIENT_SUSPENDED','MIN_TOPUP_REQUIRED','FORBIDDEN','PRODUCT_NOT_FOUND','PRODUCT_DISABLED','RATE_LIMITED','INTERNAL_ERROR','SYSTEM_DISABLED','VALIDATION_ERROR']);
export function supplierErrorDiagnostics(error) {
  return {
    revision:'supplier-upstream-diagnostics-v1',
    stage:['me','products','product','sandbox-order'].includes(error.stage)?error.stage:'local-check',
    upstreamStatus:Number.isInteger(error.upstreamStatus)?error.upstreamStatus:null,
    providerCode:providerErrors.has(error.providerCode)?error.providerCode:null,
    responseFormat:['json','html','other'].includes(error.responseFormat)?error.responseFormat:null
  };
}
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (!response.ok || envelope?.success === false) {
        const failure=safeError(status, envelope, retryAfter);
        failure.responseFormat=envelope?'json':response.headers.get('content-type')?.includes('text/html')?'html':'other';
        throw failure;
      }
    ...
    error.stage=path==='/me'?'me':path.startsWith('/products/')?'product':'products';
...
// Fixed ref intentionally reuses this one logical test across retries/reruns.
export const SANDBOX_PURCHASE_REF='olaf-sandbox-999001-verification-v1';
export async function testSandboxPurchase(options={}) {
  const env=options.env || process.env;
  const config=sandboxConfig({...env,SUPPLIER_499K_API_MODE:'sandbox'});
  ...
  const product=await requestSandbox('/products/999001?type=sandbox',options,config);
  if(String(product.product_id)!=='999001' || product.type!=='sandbox')throw new SupplierConnectionError('SANDBOX_PRODUCT_MISMATCH');
  // No automatic POST retry. On uncertainty reuse the same ref, never generate another.
  let response,body;
  try {
    response=await fetcher(`${config.base}/api/v1/orders`,{
      method:'POST',redirect:'error',signal:AbortSignal.timeout(10000),
      headers:{...supplierHeaders(env),'Content-Type':'application/json'},
      body:JSON.stringify({type:'sandbox',product_id:'999001',ref:SANDBOX_PURCHASE_REF})
    });
    body=await readEnvelope(response);
  }catch{
    throw new SupplierConnectionError('SANDBOX_ORDER_RESULT_UNKNOWN_REUSE_REF',503);
  }
  if(!response.ok || body.success===false){
    const failure=safeError(response.status,body,response.status===429?retryAfterSeconds(response.headers.get('retry-after')):0);
    failure.stage='sandbox-order';failure.responseFormat='json';throw failure;
  }
  const data=body.data;
  if(body.success!==true || !data || data.type!=='sandbox' || String(data.product_id)!=='999001'
    || !/^TEST-[A-Za-z0-9-]+$/.test(data.order_no || '') || data.ref!==SANDBOX_PURCHASE_REF)
    throw new SupplierConnectionError('SANDBOX_ORDER_RESPONSE_INVALID_REUSE_REF');
  // Do not return account credentials, balances, raw response or key material.
  return {success:true,mode:'sandbox',authentication:'PASS',product:'PASS',purchase:'PASS',
    orderNo:data.order_no,productId:'999001',ref:SANDBOX_PURCHASE_REF,idempotent:data.idempotent===true,
    simulatedAccountReceived:typeof data.account?.username==='string' && typeof data.account?.password==='string'};
}
...
export async function fetchOfflineCatalog(options = {}) {
  ...
  if (env.SUPPLIER_499K_API_MODE === 'sandbox') config = sandboxConfig(env);
    ...
    if (env.SUPPLIER_499K_BASE_URL !== 'https://store.499k-network.com' || !/^499k_live_[A-Za-z0-9_-]+$/.test(supplierKey(env))) throw new SupplierConnectionError('SUPPLIER_CONFIG_REQUIRED',503);
    config = {base:env.SUPPLIER_499K_BASE_URL,key:supplierKey(env)};
  }
