(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    root.innerHTML = `<article class="member-showcase">
      ...
      ${decoration ? `<form data-bio-form><textarea id="member-bio" aria-label="สังเขป สูงสุด 300 ตัวอักษร" placeholder="คลิกเพื่อเขียนสังเขปของคุณ…" maxlength="300" rows="1">${escape(decoration.bio)}</textarea><button type="submit">บันทึก ✓</button><span role="status" data-bio-status></span></form>` : '<p role="status">โหลดการตกแต่งไม่สำเร็จ กรุณาตรวจการติดตั้ง SQL พื้นหลังแล้วรีเฟรช</p>'}</div>
