...
const actions={catalog:'GET',product:'GET',quote:'POST',checkout:'POST',orders:'GET',order:'GET',delivery:'POST',guard:'POST',fulfill:'POST',publish:'POST',readiness:'GET'};
const publicCodes=new Set(['AUTH_REQUIRED','ADMIN_REQUIRED','ADMIN_CHECK_UNAVAILABLE','ACTIVE_ACCOUNT_REQUIRED','ORDER_NOT_FOUND','INVALID_CHECKOUT','IDEMPOTENCY_CONFLICT',
  ...
  'MIN_TOPUP_REQUIRED','CLIENT_NOT_APPROVED','CLIENT_SUSPENDED','KEY_REVOKED','SYSTEM_DISABLED','SUPPLIER_PRODUCT_MISMATCH','SUPPLIER_INVALID_RESPONSE','SUPPLIER_REQUEST_FAILED']);
export function createShopHandler({env=process.env,fetcher=fetch,db=createShopDb({env,fetcher}),shop=createSupplierShop({env,db}),admin=requireSupplierAdmin}={}){
  return async(req,res)=>{
    ...
    try{
      ...
      return res.status(retry?429:status).json({success:false,code});
    }
  };
}
