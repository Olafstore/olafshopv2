...
export function createSupplierShop({env=process.env,db=createShopDb({env}),client=create499kFulfillmentClient({env}),sync=()=>syncSupplierCatalog({env,db}),steam=getSteamMetadata}={}){
  ...
  return {
    ...
    async diagnose(){
      ...
      const checks={revision:'supplier-media-v4'};
      ...
      await check('catalog',()=>fetchOfflineCatalog({env:{...env,SUPPLIER_499K_API_MODE:'live',SUPPLIER_499K_ALLOW_LIVE_READ:'true'}}));
      await check('jurassicProduct',()=>client.getProduct('1244460'));
      return checks;
    },
