 ...
 async function load(reset=false){
  ...
  try{
   ...
   if(tab==='activity'){
    ...
    list.insertAdjacentHTML('beforeend',shown.map(r=>{const src=avatar(r.avatar),state=tab==='friends'?'friends':tab;return `<article class="friend-person"><a href="profile.html#user/${encodeURIComponent(r.profileKey||r.username)}">${src?`<img src="${esc(src)}" alt="" loading="lazy">`:'<span class="friend-avatar-placeholder">'+icon+'</span>'}<span><strong>${esc(r.nickname||r.username)}</strong><small>@${esc(r.username)}</small>${tab==='friends'?online(r.online):''}</span></a><div>${actions(state).map(([action,label])=>`<button type="button" data-action="${action}" data-username="${esc(r.profileKey||r.username)}">${label}</button>`).join('')}</div></article>`;}).join(''));
   }
